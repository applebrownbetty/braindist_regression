...
aim2simdat <- function(seed) {
  ...
  #connmats <- list()
  sim1_deg <- sim2_deg <- sim3_deg <- matrix(nrow = np,ncol = nn,dimnames = list(1:np,1:nn))  
  sim1_top20 <- sim2_top20 <- sim3_top20 <- matrix(nrow = np,ncol = nn,dimnames = list(1:np,1:nn))
...
neededfiles <- paste0("sim_",sprintf("%06d",1:10^5),".Rds")
