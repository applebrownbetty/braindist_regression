...
output_file_name <- paste0("perm_",sprintf("%06d",task_ID),".Rds")
...
dat_results <- data.frame(ID = rep(task_ID,length(sims)*length(signals)*length(distances)*2*(length(meths)+1)))
  ...
  #print(counter)
    for(distance in distances) {
      ...
      for(meth in meths) {
        ...
        dat_results[counter,c("ID","Sim","Signal_Percent","Distance","Method")] <- t(c(sprintf("%06d",task_ID),sim,signal,distance,meth))
        dat_results[counter,paste0(rownames(xxx$table),"_p")] <- xxx$table$`permutation Pr(>|t|)`
        counter <- counter + 1
      }

      dat_results[counter,c("ID","Sim","Signal_Percent","Distance","Method")] <- t(c(sprintf("%06d",task_ID),sim,signal,distance,"parametric"))
      ...
      for(meth in meths) {
        ...
        dat_results[counter,c("ID","Sim","Signal_Percent","Distance","Method")] <- t(c(sprintf("%06d",task_ID),sim,signal,distance,paste0(meth,"_adj")))
        dat_results[counter,paste0(rownames(xxx$table),"_p")] <- xxx$table$`permutation Pr(>|t|)`
        counter <- counter + 1
      }
      
      dat_results[counter,c("ID","Sim","Signal_Percent","Distance","Method")] <- t(c(sprintf("%06d",task_ID),sim,signal,distance,"parametric_adj"))
      dat_results[counter,paste0(rownames(xxx$table),"_p")] <- xxx$table$`parametric Pr(>|t|)`
      dat_results[counter,paste0(rownames(xxx$table),"_est")] <- xxx$table$Estimate
      dat_results[counter,paste0(rownames(xxx$table),"_se")] <- xxx$table$`Std. Error`
      dat_results[counter,paste0(rownames(xxx$table),"_t")] <- xxx$table$`t value`
